...
public class DeviceDlqProducer extends DeviceErroProducer{
	...
	public void sendMessage(DeviceErroDTO deviceErroDTO,int tentativa) {
		logger.info("Mensagem -> {}", deviceErroDTO);
		this.kafkaTemplate.send(this.sendMessage(deviceErroDTO, topicDeviceDlq, objectMapper,tentativa));
	}
