...
public class DeviceRetryProducer extends DeviceErroProducer{
	...
	public void sendMessage(DeviceErroDTO deviceRetryDTO,int tentativa) {
		logger.info("Mensagem -> {}", deviceRetryDTO);   
		this.kafkaTemplate.send(this.sendMessage(deviceRetryDTO, topicDeviceRetry, objectMapper,tentativa));
	}
}
